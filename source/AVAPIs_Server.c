...
void *thread_ReceiveAudio(void *arg)
{
	int SID = *((int *)arg);
	free(arg);
	int recordCnt = 0, recordFlag = 1, nResend = 0;
	AV_Client *p = &gClientInfo[SID];
	unsigned int servType;
	int avIndex = avClientStart2(SID, NULL, NULL, 30, &servType, p->speakerCh, &nResend);
...
void *thread_ForAVServerStart(void *arg)
{
	...
	if(IOTC_Session_Check(SID, &Sinfo) == IOTC_ER_NoERROR)
	{
		...
		unsigned int second = (end.tv_usec - start.tv_usec) ? (end.tv_sec - start.tv_sec) : (end.tv_sec - start.tv_sec - 1);
		unsigned int msecond = (end.tv_usec - start.tv_usec) ? (end.tv_usec - start.tv_usec) : (end.tv_usec + 1000000 - start.tv_usec);

		msecond = msecond / 100000;
...
int main(int argc, char *argv[])
{
    ...
    //AV_Set_Log_Path("/home/usr/log_file/av_server_log.txt", 0);

	connect_msg_fd = open("p2p_connect_msg.txt",O_RDWR | O_CREAT | O_APPEND,0666);
