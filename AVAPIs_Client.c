...
void *thread_Speaker(void *arg)
{
	...
	printf("Free CH[%d]\n", IOTC_Session_Get_Free_Channel(ServerInfo->SID));
	int avIndex = avServStart(ServerInfo->SID, NULL, NULL, 50, 0, ServerInfo->speakerCh);
	if(avIndex < 0)
	{
...
***/
int start_ipcam_stream(int avIndex, AV_Server *ServerInfo)
{
	int ret;
...
#if ENABLE_SPEAKER
	ioMsg.channel = ServerInfo->speakerCh;
...
void *thread_ConnectCCR(void *arg)
{
    ...
    if (ENABLE_SPEAKER)
        ServerInfo->speakerCh = IOTC_Session_Get_Free_Channel(SID);
    else
        ServerInfo->speakerCh = -1;
	...
	if(start_ipcam_stream(avIndex, ServerInfo))
	{
		...
		if ( (ret=pthread_create(&ThreadSpeaker_ID, NULL, &thread_Speaker, (void *)ServerInfo)) )
		{
...
int main(int argc, char *argv[])
{
    ...
    char *UID[MAX_SERVER_NUMBER];
	int ret;
	int nServerNum;
	int j=0;

	if(argc < 2 || argc > MAX_SERVER_NUMBER + 1)
	{
	...
	{
		nServerNum = argc-1;
		for (j = 0; j < nServerNum; j++)
            UID[j] = argv[j+1];
	}

    //set the output path of the log file when using log library.
    //AV_Set_Log_Path("/home/usr/log_file/av_client_log.txt", 0);
	...
	for(j=0; j<nServerNum; j++)
	{
        ...
        strcpy(ServerInfo->UID, UID[j]);
		if((ret = pthread_create(&ConnectThread_ID[j], NULL, &thread_ConnectCCR, (void *)ServerInfo)))
		{
